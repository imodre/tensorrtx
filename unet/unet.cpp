...
static const int OUTPUT_SIZE = 120 * 160 * 128;
...
ILayer* doubleConv(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int out_ch, int mid_ch, std::string lname)
{
    IConvolutionLayer* conv0 = network->addConvolutionNd(input, mid_ch, DimsHW{3, 3}, weightMap[lname + ".0.weight"], weightMap[lname + ".0.bias"]);
    assert(conv0);
    conv0->setPaddingNd(DimsHW{1, 1});
    IScaleLayer* bn1 = addBatchNorm2d(network, weightMap, *conv0->getOutput(0), lname + ".1", 1e-5);
    IActivationLayer* relu2 = network->addActivation(*bn1->getOutput(0), ActivationType::kRELU);
    assert(relu2);
    IConvolutionLayer* conv3 = network->addConvolutionNd(*relu2->getOutput(0), out_ch, DimsHW{3, 3}, weightMap[lname + ".3.weight"], weightMap[lname + ".3.bias"]);
    assert(conv3);
    conv3->setPaddingNd(DimsHW{1, 1});
    IScaleLayer* bn4 = addBatchNorm2d(network, weightMap, *conv3->getOutput(0), lname + ".4", 1e-5);
    IActivationLayer* relu5 = network->addActivation(*bn4->getOutput(0), ActivationType::kRELU);
    assert(relu5);
    return relu5;
}

ILayer* down(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int out_ch, std::string lname)
{
    auto pool0 = network->addPoolingNd(input, PoolingType::kMAX, DimsHW{2, 2});
    assert(pool0);
    pool0->setStrideNd(DimsHW{2, 2});
    auto inc = doubleConv(network, weightMap, *pool0->getOutput(0), out_ch, out_ch, lname + ".maxpool_conv.1.double_conv");
    return inc;
}
...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt)
{
    ...
    auto inc = doubleConv(network, weightMap, *data, 64, 64, "inc.double_conv");
    auto down1 = down(network, weightMap, *inc->getOutput(0), 128, "down1");
    ...
    down1->getOutput(0)->setName(OUTPUT_BLOB_NAME);
    std::cout << "set name out" << std::endl;
    network->markOutput(*down1->getOutput(0));
...
int main(int argc, char** argv)
{
    if (argc != 2) {
        ...
        std::cerr << "./unet -d   // deserialize plan file and run inference" << std::endl;
        return -1;
    }
    ...
    if (std::string(argv[1]) == "-s") {
        ...
        std::ofstream p("unet.engine", std::ios::binary);
        if (!p)
        {
        ...
        std::ifstream file("unet.engine", std::ios::binary);
        if (file.good()) {
